...
`
  ...
  await expect(page.getByRole("heading", { name: "Data quality report", exact: false })).toBeVisible()
  await expect(page.getByText("100%", { exact: true })).toHaveCount(3)
  await expect(page.locator(".data-quality-overview")).toContainText("0records need attention")
  const qualityDownload = page.waitForEvent("download")
  await page.getByRole("button", { name: "Download cleanup report" }).click()
  const qualityCsv = await (await qualityDownload).createReadStream()
  let qualityContents = ""
  for await (const chunk of qualityCsv) qualityContents += chunk.toString()
  expect(qualityContents).toContain("Record type,Record,Completeness (%)")
  expect(qualityContents).toContain("Material,Recycled blend,100,medium,1 year")
  expect(qualityContents).toContain("Process,Material production,100")
  await expect(page.getByRole("combobox", { name: "Filter by lifecycle stage" })).toHaveValue("all")
  await page.getByRole("combobox", { name: "Filter by lifecycle stage" }).selectOption("manufacturing")
  await expect(page.locator(".processes-table tbody tr")).toContainText("Material production")
  await page.getByRole("combobox", { name: "Filter by lifecycle stage" }).selectOption("all")
  await page.getByRole("combobox", { name: "Filter by material family" }).selectOption("polymer")
  ...
  await expect(page.locator(".materials-table tbody tr")).toContainText("Recycled blend")
  await expect(page.locator(".processes-table")).toContainText("No processes match the current filters")
  const filteredProcessDownload = page.waitForEvent("download")
  await page.getByRole("button", { name: "Download filtered processes CSV" }).click()
  const filteredProcessCsv = await (await filteredProcessDownload).createReadStream()
  let filteredProcessContents = ""
  for await (const chunk of filteredProcessCsv) filteredProcessContents += chunk.toString()
  expect(filteredProcessContents).toContain("Process,Lifecycle stage,Location")
  expect(filteredProcessContents).not.toContain("Material production")
